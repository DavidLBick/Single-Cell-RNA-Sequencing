...
# usage: omp_feature_selection <data_path> <n_features_to_select> <n_rows (optional)>
...
def add_or_remove_some(n_classes, current_amount, target_n, new_set):
	n_features_to_add_or_remove
...
def prune_to_correct_amount(selections, target_n):
	...
	while n_features_not_close_enough(n_classes, current_amount, target_n):
		if current_amount > target_n:
			new_set, current_amount = remove_some(n_classes, current_amount, target_n, selections)
		elif current_amount < target_n:
			new_set
		new_set, current_amount = add_or_remove_some(n_classes, current_amount, target_n, new_set)
